...
def create_survey_response(user_id: int, survey_name: str, optimal_allocation: list) -> int:
    ...
    """
    query = """
        INSERT INTO survey_responses (user_id, survey_name, optimal_allocation)
        VALUES (%s, %s, %s)
    """
    optimal_allocation_json = json.dumps(optimal_allocation)
    logger.debug(f"Executing query: {query} with user_id: {user_id}, survey_name: {survey_name}, optimal_allocation: {optimal_allocation_json}")
    return execute_query(query, (user_id, survey_name, optimal_allocation_json))
