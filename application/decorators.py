            ...
            if not user_id or not external_survey_id:
                ...
                if custom_internal_id:
                    try:
                        ...
                        return redirect(url_for("survey.index", **request.args))
                ...
                if not is_eligible:
                    ...
                    return redirect(
                        url_for(f"survey.{redirect_url}", **redirect_kwargs)
                    )
                else:
                    should_call_f = True

        except Exception as e:
            logger.error(f"Error in survey eligibility check: {str(e)}", exc_info=True)
            return redirect(url_for("survey.index", **request.args))

        # Call the decorated function with its original arguments OUTSIDE the try-except block
        if should_call_f:
