...
class AsymmetricLossDistributionStrategy(PairGenerationStrategy):
    """
    Strategy testing preferences for concentrated vs. distributed changes.
    ...
    Algorithm:
    1. Calibrated Magnitude: base_unit = max(1, round(min(ideal) / 10))
    ...
    4. Two Comparison Types:
       ...
       - Type B (Fallback): fixed difference vectors with cyclic rotations.
    """
    ...
    def generate_pairs(
        ...
        for target_idx in range(vector_size):
            for level_idx, x in enumerate(x_levels, 1):
                ...
                magnitude_value = x  # Default for Type A
                if (
                    np.all(vec_a1 >= 0)
                    and np.all(vec_a2 >= 0)
                    ...
                    pair_type = "A"
                    option1_vec, option2_vec = (
                        tuple(vec_a1.tolist()),
                        tuple(vec_a2.tolist()),
                    )
                else:
                    # Type B (Fallback): fixed difference vectors with rotation
                    pair_type = "B"
                    ...
                    if not (
                        ...
                        logger.warning(
                            "Could not generate valid Type B pair for "
                            f"user_vector={user_vector}, "
                    ...
                    # Magnitude for Type B equals absolute change
                    # at the target index
                    magnitude_value = abs(diff_concentrated[target_idx])
                ...
                option1_strategy = (
                    f"{concentrated_label} (" f"{magnitude_value}, Type {pair_type})"
                )
                option2_strategy = (
                    f"{distributed_label} (" f"{magnitude_value}, Type {pair_type})"
                )
