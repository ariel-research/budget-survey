...
def index():
    ...
    This function checks for a participant_id in the URL parameters.
    If not present, it renders an error page.
    """
    participant_id = request.args.get('participant_id')
    if not participant_id:
        logger.warning("Index page accessed without participant_id")
        return render_template('error.html', message="מזהה המשתמש אינו נמצא. צור קשר לקבלת לינק תקין, אשר מכיל את מזהה המשתמש.")
    logger.info(f"Index page accessed by participant {participant_id}")
...
def create_vector():
    ...
    participant_id = request.args.get('participant_id')
    if not participant_id:
        logger.warning("Attempt to access create_vector without participant_id")
        return redirect(url_for('main.index'))

    if request.method == 'POST':
        # Extract the user vector from the form data
        user_vector = [int(request.form.get(f'subject_{i}', 0)) for i in range(3)]
        logger.debug(f"User {participant_id} submitted vector: {user_vector}")
        
        if sum(user_vector) != 100 or any(v % 5 != 0 for v in user_vector):
            error_message = "הסכום חייב להיות 100 וכל מספר חייב להתחלק ב-5."
            logger.warning(f"Invalid vector submitted by user {participant_id}: {user_vector}")
            return render_template('create_vector.html', error=error_message, subjects=SUBJECTS, participant_id=participant_id)
        
        logger.info(f"Valid vector created by user {participant_id}: {user_vector}")
        # Redirect to survey page with user's vector and ID in URL parameters
        return redirect(url_for('main.survey', vector=','.join(map(str, user_vector)), participant_id=participant_id))
    
    logger.debug(f"Create vector page accessed by user {participant_id}")
    return render_template('create_vector.html', subjects=SUBJECTS, participant_id=participant_id)
...
def survey():
    ...
    participant_id = request.args.get('participant_id')
    if not participant_id:
        logger.warning("Attempt to access survey without participant_id")
        return redirect(url_for('main.index'))

    if request.method == 'GET':
        # Converts the vector back into a tuple of integers
        user_vector = tuple(map(int, request.args.get('vector', '').split(',')))
        logger.debug(f"Survey accessed by user {participant_id} with vector: {user_vector}")
        if len(user_vector) != 3 or sum(user_vector) != 100:
            logger.warning(f"Invalid vector in survey GET for user {participant_id}: {user_vector}")
            return redirect(url_for('main.create_vector', participant_id=participant_id))
        ...
        logger.info(f"Survey generated for user {participant_id} with vector: {user_vector}")
        ...
        logger.debug(f"Survey submission received from user {participant_id}. User vector: {user_vector}")
        ...
        try:
            ...
            logger.info(f"Survey marked as completed for user {participant_id}: {survey_response_id}")
        except Exception as e:
            logger.error(f"Error processing survey submission for user {participant_id}: {str(e)}", exc_info=True)
